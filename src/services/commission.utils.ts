interface CommissionTier {
  startPrice: number
  endPrice: number // Use Infinity for no upper limit
  commission: number // Fixed commission amount
}

type CommissionTable = CommissionTier[][]

class ReferralService {
  // Predefined commission table (1-based index = referral level)
  private commissionTable: CommissionTable = [
    [], // Level 0 (unused)
    [
      // Level 1 commissions
  ...
  calculateReferralCommission(
    ...
    if (
      referralLevel >= this.commissionTable.length ||
    ...
    const applicableTier = levelTiers.find(
      tier => totalPrice >= tier.startPrice && totalPrice < tier.endPrice
    )
...
export { CommissionTable, CommissionTier }
